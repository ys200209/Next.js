...
export default function ShowChart() {
    ...
    // 컴포넌트가 마운트된 후에 init 함수를 호출
    useEffect(() => {
        init();
    }, []); // 빈 배열을 전달하여 컴포넌트가 처음 한 번만 마운트될 때 호출

    return (
        <>
            <div id="myDiagramDiv" style={diagramStyle}></div>
        </>
    );
}
...
function init() {
    ...
    // 이미 div에 다이어그램이 연결됐다면, 삭제하고 새로운 다이어그램을 생성하는 방법:
    ...
    if (myDiagram) {
        ...
        myDiagram.div = null; // Unbind the existing diagram from the div
    }
...
function init() {
    ...
    function buttonExpandCollapse(e: go.InputEvent, port: go.GraphObject) {
        const node = port.part as go.Node;
        node.diagram!.startTransaction("expand/collapse");
        const portid = port.portId as string;
        node.findLinksOutOf(portid).each((l: go.Link) => {
            if (l.visible) {
    ...
    function collapseTree(node: go.Node, portid: string | null) {
        node.findLinksOutOf(portid).each((l: go.Link) => {
            l.visible = false;
            const n = l.getOtherNode(node);
            if (n !== null) {
    ...
    const model = new go.GraphLinksModel({ linkFromPortIdProperty: "fromport" });
...
function makeNodes(model: go.GraphLinksModel) {
    var nodeDataArray = [
...
function makeLinks(model: go.GraphLinksModel) {
    var linkDataArray = [];
